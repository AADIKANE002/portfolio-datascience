...
const Terminal = ({ isOpen, onClose, onOpenResume }) => {
  ...
  const [history, setHistory] = useState([
    {
      type: 'system',
      text: `Aditya Kumar [Data Science & GenAI CLI v2.4.0]
Type "help" to see available commands or click quick action buttons below.`
  ...
  const handleCommand = (cmdStr) => {
    ...
    if (trimmed === 'resume') {
      ...
      newHistory.push({
        type: 'response',
        text: 'Opening Data Scientist resume preview...'
      });
      setHistory(newHistory);
      setInput('');
      return;
    ...
    if (trimmed === 'whoami') {
      newHistory.push({
        type: 'response',
        text: 'guest@data-explorer (Recruiter / Data Science Leader)'
      });
      ...
      <div
        className={`relative w-full max-w-3xl h-[80vh] sm:h-[620px] rounded-2xl flex flex-col overflow-hidden border shadow-2xl transition-all ${
          matrixMode
            ...
            : 'bg-space-950/95 border-emerald-500/40 shadow-emerald-500/10 text-slate-200'
        }`}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-space-900 border-b border-white/10 select-none">
          ...
          <div className="flex items-center gap-2 text-xs font-mono font-medium text-slate-400">
            ...
            <span>aditya@datascience-box:~ (Interactive CLI)</span>
          </div>
          ...
          {history.map((item, index) => (
            <div key={index} className="space-y-1">
              {item.type === 'user' ? (
                ...
                <div className="text-slate-300 whitespace-pre-wrap pl-2 border-l-2 border-emerald-500/40">
                  {item.text}
                </div>
              )}
            </div>
        ...
        <div className="p-3 bg-space-900/80 border-t border-white/10 flex flex-wrap items-center gap-1.5">
          ...
          {quickCommands.map((cmd) => (
            <button
              ...
              className="px-2.5 py-1 text-[11px] font-mono rounded-md bg-white/5 hover:bg-emerald-500/20 text-slate-300 hover:text-emerald-300 border border-white/10 hover:border-emerald-500/40 transition-all"
            >
              {cmd}
            </button>
          ))}
